...
#define METADATA_SIZE sizeof(int) + sizeof(char)
...
matrice create_matrice(int size) {
	...
	m[0] = size;
	*((char *)(void *)m - sizeof(int)) = 0;

	return (char *)((void *)m + METADATA_SIZE);
}
char set_char(matrice m, char val) {
	return *((char *)(void *)m - sizeof(int)) = val;
}
...
int get_size(matrice m) {
	return *(char *)((void *)m - sizeof(int) - sizeof(char));
}
...
TurnEnum get_turn(matrice m) {
	return *((char *)((void *)m - sizeof(int)));
}
TurnEnum switch_turn(matrice m) {
	TurnEnum turn = get_turn(m);
	return *((char *)((void *)m - sizeof(int))) = turn == White ? Black : White;
}
